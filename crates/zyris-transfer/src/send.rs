...
//! `zyris-transfer` implements a capability, `zyris-p2p` carries it.
...
impl LocalFileTransfer {
    ...
    /// to `listen::serve_peers` as its directory — one client, kept current in one place,
    ...
    ///
    /// [`zyris_caps::resolve_under`] is explicitly not a jail — its own doc says "the root is a
    ...
    async fn resolve_source(&self, path: &str) -> Result<PathBuf> {
        ...
        let requested = zyris_caps::resolve_under(&root, path);
